...
const users_mongo_1 = require("../../models/users/users.mongo");
const DEFAULT_USER_ID = 0;
async function getLatestUserId() {
    const latestUser = await users_mongo_1.userModel.findOne().sort("-userId");
    if (!latestUser) {
...
async function saveAccount(user) {
    try {
        await users_mongo_1.userModel.findOneAndUpdate({
            userId: user.userId,
        }, user, { upsert: true });
    }
    catch (err) {
...
async function getAllUsers() {
    try {
        return await users_mongo_1.userModel.find({}, { _id: 0, __v: 0, password: 0, salt: 0, active: 0 });
    }
    catch (err) {
        console.error(err);
    }
...
async function getUserById(userId) {
    try {
        return await users_mongo_1.userModel.findOne({ userId: userId }, { _id: 0, __v: 0, password: 0, salt: 0, active: 0 });
    }
    catch (err) {
        console.error(err);
    }
